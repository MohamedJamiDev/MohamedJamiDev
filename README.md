# 👋 Hi there, I'm Mohamed!

🚀 **Aspiring Software Developer | Lifelong Learner | Tech Enthusiast**

I'm a passionate and curious developer continuously learning and building projects to break into tech. With a strong foundation in **Java, Spring Boot, SQL, and software testing**, I am dedicated to expanding my knowledge, refining my skills, and gaining hands-on experience through real-world projects.

---
## 🔹 What I'm Working On
- 💻 **Trading Calculator App** – A Spring Boot web app to help day traders track their **profit & loss** effectively.
- 🛍 **E-commerce Test Automation** – Automating testing processes using **Selenium with Java** to ensure software reliability and efficiency.

---
## 💡 My Learning Journey
- 🎓 **Completed Software Testing Bootcamp** at **PeopleNTech** – Gained hands-on experience testing web applications using **Java Selenium**, Jira, and Jenkins, while working in an **Agile environment**.
- 📚 **Currently enrolled in** Amazon's **Junior Software Developer Professional Certificate** on Coursera – Enhancing my **software development skills** while building projects to gain practical experience.

---

# 💻 Tech Stack:
![Java](https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white) ![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white) ![Spring](https://img.shields.io/badge/spring-%236DB33F.svg?style=for-the-badge&logo=spring&logoColor=white) ![MySQL](https://img.shields.io/badge/mysql-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white) ![Jenkins](https://img.shields.io/badge/jenkins-%232C5263.svg?style=for-the-badge&logo=jenkins&logoColor=white) ![Apache Maven](https://img.shields.io/badge/Apache%20Maven-C71A36?style=for-the-badge&logo=Apache%20Maven&logoColor=white) ![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white) ![Jira](https://img.shields.io/badge/jira-%230A0FFF.svg?style=for-the-badge&logo=jira&logoColor=white)

<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->

---
## 🎯 My Goals for 2025
- ✅ Build and deploy a **full-stack web application**
- ✅ Master **Spring Boot and backend development**
- ✅ Contribute to **open-source projects**
- ✅ Land my **first tech role!**


